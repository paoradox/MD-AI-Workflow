...
Single-file AI operating spec combining consistent coding behavior and an output shape built for fast scanning and low friction between "got it" and "done it." Minimizes repetitive prompting, preserves approved context across a session, and maximizes first-attempt accuracy — without maintaining separate workflow and communication-style files. README generation lives in a separate standalone spec, `README_GENERATION.md`.
...
README generation is not covered by this file — see `README_GENERATION.md`.
